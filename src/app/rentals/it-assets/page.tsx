  ...
  return (
    <div className="container animate-fade-in pb-12">
      ...
      <div className="filter-bar animate-slide-up" style={{ position: 'relative', zIndex: 10 }}>
        <div className="search-box flex-1 min-w-[240px]">
          ...
          <input 
            id="rent_search"
            type="text" 
            placeholder="Cari item, serial, billing ref, vendor..." 
            value={search} 
      ...
      {error ? (
        ...
        <>
          <TableShell headers={[{label:'Vendor Billing Ref'},{label:'Item / Spesifikasi'},{label:'Vendor'},{label:'Serial / Unit'},{label:'Masa Sewa'},{label:'Biaya (Rp)',right:true},{label:'Status',right:true},{label:'Aksi',right:true}]} loading={loading} colSpan={8}>
            {rows.length===0 ? (
      ...
      {/* Add/Edit */}
      {(showAdd||editRow)&&(
        <ModalShell title={editRow?`Edit Rental IT`:'Tambah Rental IT'} onClose={closeForm} size="md" closeOnClickOutside={false}>
          <div className="flex flex-col gap-4">
            ...
            <div className="detail-grid">
              ...
              {/* BRAND */}
              <FF label="Brand" id="rnt_brand" required>
                <input id="rnt_brand" type="text" value={form.brand} onChange={e=>sf('brand',e.target.value)} placeholder="e.g. Lenovo, Dell, Apple" className="input-premium" title="Brand" />
              </FF>
              
              {/* MODEL UNIT */}
              <FF label="Model Unit" id="rnt_model" required>
                <input id="rnt_model" type="text" value={form.model} onChange={e=>sf('model',e.target.value)} placeholder="e.g. ThinkPad L14 Gen 2" className="input-premium" title="Model Unit" />
              </FF>

              {/* TIPE PROCESSOR */}
              <FF label="Tipe Processor" id="rnt_processor">
                <input id="rnt_processor" type="text" value={form.processor} onChange={e=>sf('processor',e.target.value)} placeholder="e.g. Intel Core i5 / Apple M3" className="input-premium" title="Tipe Processor" />
              </FF>

              {/* OS */}
              <FF label="Sistem Operasi (OS)" id="rnt_os">
                <input id="rnt_os" type="text" value={form.os} onChange={e=>sf('os',e.target.value)} placeholder="e.g. Windows 11 Pro / macOS" className="input-premium" title="Sistem Operasi" />
              </FF>

              {/* RAM */}
              <FF label="Kapasitas RAM" id="rnt_ram">
                <input id="rnt_ram" type="text" value={form.ram} onChange={e=>sf('ram',e.target.value)} placeholder="e.g. 8GB / 16GB DDR4" className="input-premium" title="Kapasitas RAM" />
              </FF>

              {/* STORAGE */}
              <FF label="Penyimpanan (Storage)" id="rnt_storage">
                <input id="rnt_storage" type="text" value={form.storage} onChange={e=>sf('storage',e.target.value)} placeholder="e.g. 256GB SSD / 512GB NVM" className="input-premium" title="Penyimpanan (Storage)" />
              </FF>

              <FF label="Tipe Perangkat" id="rnt_type">
                <select id="rnt_type" value={form.device_type} onChange={e=>sf('device_type',e.target.value)} className="input-premium" title="Tipe Perangkat">
              ...
              <FF label="Vendor Billing Ref" id="rnt_order"><input id="rnt_order" type="text" value={form.order_id} onChange={e=>sf('order_id',e.target.value)} className="input-premium" title="Vendor Billing Ref" /></FF>
            ...
            <div className="modal-footer-border">
              ...
              <button type="button" className="btn btn-primary min-w-130" onClick={save} disabled={saving} title={editRow?'Simpan Perubahan':'Tambah Rental IT'}>
      ...
      {deleteItem && (
        ...
        >
          <div className="flex flex-col gap-3 text-center items-center py-2">
            ...
            <h3 className="text-md font-800 text-text">Hapus Rental IT?</h3>
