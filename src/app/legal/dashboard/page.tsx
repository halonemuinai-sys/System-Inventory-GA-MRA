...
import {
  ...
  ShieldAlert, AlertTriangle, TrendingUp, FileText, Activity, Zap, Building2,
} from 'lucide-react';
import {
...
// ── Module config ─────────────────────────────────────────────
const MODULE_CFG = {
  contract:   { label:'Contract Management', Icon: FileSignature, gradient:'#3b82f6,#2563eb', badge:'badge-indigo'  },
  corporate:  { label:'Corporate Legal',     Icon: Building2,     gradient:'#8b5cf6,#7c3aed', badge:'badge-violet'  },
  litigation: { label:'Litigation & Dispute',Icon: Gavel,         gradient:'#6366f1,#4f46e5', badge:'badge-indigo'  },
} as const;
type ModuleKey = keyof typeof MODULE_CFG;

const BADGE_MAP: Record<string, string> = {
  contract: 'badge-indigo', corporate: 'badge-violet', litigation: 'badge-indigo',
};
...
export default function LegalDashboardPage() {
  ...
  // Alert banner data — sorted by urgency
  const alertDocs = [...data.criticalDocs].sort((a, b) => parseInt(a.days_until_expiry) - parseInt(b.days_until_expiry));
        ...
        {alertDocs.length > 0 && (
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[11px] font-800 text-slate-400 uppercase tracking-widest">Dokumen Perlu Perhatian</span>
              <span className="bg-rose-500 text-white text-[10px] font-800 px-2 py-0.5 rounded-full animate-pulse">{alertDocs.length}</span>
            </div>
            <div className="flex gap-3 overflow-x-auto pb-2">
              {alertDocs.map(doc => {
                const days = parseInt(doc.days_until_expiry);
                const isExp = days < 0;
                const accent = isExp || days <= 7 ? '#ef4444' : days <= 14 ? '#f97316' : days <= 30 ? '#eab308' : '#3b82f6';
                const bg     = isExp || days <= 7 ? '#fff5f5' : days <= 14 ? '#fff7ed' : days <= 30 ? '#fefce8' : '#eff6ff';
                const txt    = isExp || days <= 7 ? 'text-rose-600' : days <= 14 ? 'text-orange-600' : days <= 30 ? 'text-amber-600' : 'text-blue-600';
                const dayLabel = isExp ? `Exp ${Math.abs(days)}h lalu` : days === 0 ? 'Hari ini!' : `${days} hari lagi`;
                const modName = MODULE_CFG[doc.module as ModuleKey]?.label ?? doc.module;
                const badge   = BADGE_MAP[doc.module] ?? 'badge-slate';
                return (
                  <div
                    key={doc.id}
                    className="shrink-0 rounded-xl p-3"
                    style={{ minWidth: 220, background: bg, borderLeft: `3px solid ${accent}`, border: `1px solid ${accent}22`, borderLeftWidth: 3, borderLeftColor: accent }}
                  >
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <Badge label={modName} colorClass={badge}/>
                      <span className={`text-[10px] font-800 shrink-0 ${txt}`}>{dayLabel}</span>
                    </div>
                    <p className="text-[11px] font-700 text-slate-800 truncate mb-1" title={doc.doc_name}>{doc.doc_name}</p>
                    <p className="text-[10px] text-slate-400">{doc.category} · PIC: {doc.pic}</p>
                  </div>
                );
              })}
            </div>
        ...
        >
          ...
          <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="flex items-start gap-4">
              <div
                className="w-14 h-14 rounded-2xl bg-blue-600 flex items-center justify-center shrink-0"
            ...
            return (
              ...
              >
                ...
                <div className="w-full h-1 rounded-full bg-white/20">
                  <div className="h-full rounded-full bg-white/70 transition-all duration-1000" style={{ width:`${Math.min(pct, 100)}%` }}/>
                </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {data.criticalDocs.map((doc) => {
                    ...
                    const dayLabel = days < 0
                      ...
                      : `${days} hari`;
                    const badge = BADGE_MAP[doc.module] ?? 'badge-slate';
                    const modLabel = MODULE_CFG[doc.module as ModuleKey]?.label ?? doc.module;
                    return (
                      <tr key={doc.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-4 py-3"><Badge label={modLabel} colorClass={badge}/></td>
