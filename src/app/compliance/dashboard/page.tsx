...
import {
  AlertCircle, RefreshCw, LayoutDashboard, ShieldAlert, AlertTriangle,
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {alertDocs.map(doc => {
                ...
                const isExp  = days < 0;
                const accent = isExp || days <= 7 ? '#ef4444' : days <= 14 ? '#f97316' : days <= 30 ? '#eab308' : '#6366f1';
                const bg     = isExp || days <= 7 ? '#fff5f5' : days <= 14 ? '#fff7ed' : days <= 30 ? '#fefce8' : '#f5f3ff';
                const txt    = isExp || days <= 7 ? 'text-rose-600' : days <= 14 ? 'text-orange-600' : days <= 30 ? 'text-amber-600' : 'text-violet-600';
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <Badge label={cfg?.label ?? doc.module} colorClass={cfg?.badge ?? 'badge-slate'}/>
        ...
        >
          ...
          <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="flex items-start gap-4">
              <div
                className="w-14 h-14 rounded-2xl bg-emerald-600 flex items-center justify-center shrink-0"
            ...
            <div className="chart-reveal" style={{ animationDelay:'100ms' }}>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={data.byExpiryStatus} margin={{ top:8, right:16, bottom:24, left:0 }}>
                  ...
                  <XAxis dataKey="label" tick={{ fontSize:10, fill:'#64748b' }} axisLine={false} tickLine={false} angle={-30} textAnchor="end" interval={0}/>
