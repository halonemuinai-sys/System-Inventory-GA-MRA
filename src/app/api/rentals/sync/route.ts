...
export async function POST() {
  ...
  try {
    ...
    const condRes = await query('SELECT id, name FROM m_condition');
    const statRes = await query('SELECT id, name FROM m_status');
    ...
    const gaUsersRes = await query('SELECT id, email FROM m_user WHERE is_active = true');
    ...
    let itCatId = null;
    const catRes = await query('SELECT id FROM m_asset_category WHERE name = $1', [itCatName]);
    if (catRes.rows.length > 0) {
      itCatId = catRes.rows[0].id;
    } else {
      const newCat = await query('INSERT INTO m_asset_category (name) VALUES ($1) RETURNING id', [itCatName]);
    ...
    for (const asset of hdAssets) {
      ...
      if (asset.ownershipType === 'RENTAL') {
        ...
        const checkRes = await query('SELECT id FROM device_rentals WHERE unit_code = $1', [asset.assetTag]);
        
        if (checkRes.rows.length > 0) {
          await query(`
            UPDATE device_rentals SET
              company_id = $1, device_type = $2, order_id = $3, item_name = $4,
              price = $5, duration_months = $6, start_rent = $7, end_rent = $8,
              user_id = $9, department = $10, status = $11
            WHERE id = $12
          `, [
            gaCompanyId, device_type, asset.vendorRef || null, item_name,
            asset.rentalCost || 0, duration_months, asset.rentalStart || null,
            asset.rentalEnd || null, gaUserId, asset.user_dept || null, rentalStatus,
            checkRes.rows[0].id
          ]);
        } else {
          await query(`
            INSERT INTO device_rentals (
          ...
          `, [
            gaCompanyId, device_type, asset.vendorRef || null, item_name,
            asset.rentalCost || 0, asset.assetTag, duration_months,
            asset.rentalStart || null, asset.rentalEnd || null, gaUserId,
            asset.user_dept || null, rentalStatus
          ]);
        }
        rentalSyncCount++;
      } else {
        ...
        let typeId = null;
        const typeCheck = await query('SELECT id FROM m_asset_type WHERE category_id = $1 AND name = $2', [itCatId, typeName]);
        if (typeCheck.rows.length > 0) {
          typeId = typeCheck.rows[0].id;
        } else {
          const newType = await query('INSERT INTO m_asset_type (category_id, name) VALUES ($1, $2) RETURNING id', [itCatId, typeName]);
        ...
        await query(`
          ...
          ON CONFLICT (asset_code) DO UPDATE SET
            ...
            updated_at = NOW()
        `, [
          gaCompanyId, asset.assetTag, itCatId, typeId, `${brand} ${model}`,
          detailsText, gaUserId, asset.rentalStart || null, asset.rentalCost || 0,
          conditionId, statusId, asset.notes || null
        ]);
        ownedSyncCount++;
      }
    }
    ...
    return NextResponse.json({ 
      success: true, 
      rentalCount: rentalSyncCount,
      ownedCount: ownedSyncCount
    });
